...
const Blog = ({ blog }) => {
  ...
  const updateBlogMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setInfo(
        ...
        `You liked the blog '${blog.title}' by '${blog.author}'.`,
      )
    },
    onError: (error) => {
  ...
  const deleteBlogMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setInfo(
        notificationDispatch,
        `You deleted the blog '${blog.title}' by '${blog.author}'.`,
      )
    },
    onError: (error) => {
      setError(notificationDispatch, error.response.data.error)
  ...
  return (
    <Card shadow="md" padding="md" radius="md" withBorder>
      ...
      <Group mt="sm">
        ...
        <Anchor href={blog.url} target="_blank" rel="noreferrer">
          {blog.url}
        </Anchor>
      </Group>
      ...
      {user.username && user.username === blog.user?.username && (
        <Button onClick={() => handleBlogDeleted(blog)} mt="sm" variant="light">
          Remove
        </Button>
      )}
    </Card>
